...
def BPR_train_original(dataset, recommend_model, loss_class, epoch, neg_k=1, w=None):
    ...
    return f"loss{aver_loss:.5f}-{time_info}", aver_loss
...
def BPR_train_ode_original(dataset, recommend_model, loss_class, epoch, neg_k=1, w=None):
    ...
    train_time = perf_counter()-t
    print("Train time: {:.4f}s".format(train_time))
    return f"loss{aver_loss:.5f}-{time_info}", times
...
def BPR_train_ode(dataset, recommend_model, loss_class, epoch, neg_k=1, w=None):
    ...
    train_time = perf_counter()-t
    print("Train time: {:.4f}s".format(train_time))
    return f"loss{aver_loss:.5f}-{time_info}", [times_0, times_1, times_2], aver_loss
...
def BPR_train_ode_t2(dataset, recommend_model, loss_class, epoch, neg_k=1, w=None):
    ...
    time_info = timer.dict()
    timer.zero()
    return f"loss{aver_loss:.5f}-{time_info}", [times_0, times_1], aver_loss
...
def BPR_train_ode_t1(dataset, recommend_model, loss_class, epoch, neg_k=1, w=None):
    ...
    time_info = timer.dict()
    timer.zero()
    return f"loss{aver_loss:.5f}-{time_info}", times_0
...
def Test(dataset, Recmodel, epoch, w=None, multicore=0):
    ...
    with torch.no_grad():
        ...
        for batch_users in utils.minibatch(users, batch_size=u_batch_size):
            ...
            rk_arr = rating_K.cpu().numpy()
            proba = [[rating[i][item_idx] for item_idx in rki] for i, rki in enumerate(rk_arr)]
        ...
        if multicore == 1:
            pool.close()
        print(results)
        return results, users_list, rating_list, proba_list
