...
save_name = time.strftime("%m-%d-%Hh%Mm-") + "-" + world.dataset + "-" + world.model_name + "-" + world.config['solver'] + "-adjoint_" + str(world.adjoint) + "-learnable_t_" + str(world.config['learnable_time']) + "-dual_res_" + str(world.config['dual_res']) + "-lr" + str(world.config['lr']) + "-lr_time"
...
try:
    for epoch in range(world.TRAIN_epochs+1):
        ...
        # if epoch %10 == 0:
            # cprint("[TEST]")
            # _, users, ratings= Procedure.Test(dataset, Recmodel, epoch, w, world.config['multicore'])
        ...
        if world.model_name == 'ltocf':
            ...
            if world.config['learnable_time'] == False:
                ...
                output_information, times_list, avg_loss = Procedure.BPR_train_ode_t2(dataset, Recmodel, bpr_t, epoch, neg_k=Neg_k,w=w)
                print(f'EPOCH[{epoch+1}/{world.TRAIN_epochs}] {output_information}')
                print(times_list)        
        ...
        epochs = [319]
        if epoch in epochs:
            results, users, ratings, probas= Procedure.Test(dataset, Recmodel, epoch, w, world.config['multicore'])

            DATA_NAME = world.dataset
            utils.save_rating_user(
                ratings, users,name=f"_{DATA_NAME}_{world.config['latent_dim_rec']}_{avg_loss:.4f}_",
                    epoch=epoch, recall=results["recall"][0]
            )
            utils.save_proba(
                probas, name=f"_{DATA_NAME}_{world.config['latent_dim_rec']}_{avg_loss:.4f}_",
                    epoch=epoch, recall=results["recall"][0]
            )
            continue
